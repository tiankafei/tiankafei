...
public class ExpressionLexer {
    ...
    public Token<?> scan(final boolean analyse) {
        ...
        if (Character.isDigit(this.peek) && this.peek == AviatorConstants.ZERO) {
            nextChar();
            if (this.peek == AviatorConstants.CHAR_X_LOWER_CASE || this.peek == AviatorConstants.CHAR_X_UPER_CASE) {
                nextChar();
        ...
        // If it is a digit
        if (Character.isDigit(this.peek) || this.peek == AviatorConstants.POINT) {
            StringBuffer sb = new StringBuffer();
            ...
            do {
                sb.append(this.peek);
                if (this.peek == AviatorConstants.POINT) {
                ...
                } else if (this.peek == AviatorConstants.CHAR_N_UPER_CASE) {
                    // big integer
                    if (hasDot) {
                        throw new CompileExpressionErrorException(
                                "Illegal number " + sb + " at " + this.iterator.getIndex());
                    ...
                    nextChar();
                    break;
                } else if (this.peek == AviatorConstants.CHAR_M_UPER_CASE) {
                    isBigDecimal = true;
                    nextChar();
                    break;
                } else if (this.peek == AviatorConstants.CHAR_E_LOWER_CASE || this.peek == AviatorConstants.CHAR_E_UPER_CASE) {
                    if (scientificNotation) {
                        throw new CompileExpressionErrorException(
                                "Illegal number " + sb + " at " + this.iterator.getIndex());
                    }
                    ...
                    nextChar();
                    if (this.peek == AviatorConstants.HORIZONTAL_LINE) {
                        negExp = true;
                        sb.append(this.peek);
                        nextChar();
                    }
        ...
        // It is a variable
        if (this.peek == AviatorConstants.HASHTAG) {
            int startIndex = this.iterator.getIndex();
            nextChar(); // skip $
            StringBuilder sb = new StringBuilder();
            while (Character.isJavaIdentifierPart(this.peek) || this.peek == AviatorConstants.POINT || this.peek == AviatorConstants.LEFT_MIDDLE_BRACKET
                    || this.peek == AviatorConstants.RIGHT_MIDDLE_BRACKET) {
                sb.append(this.peek);
                nextChar();
            }
            String lexeme = sb.toString();
        ...
        // String
        if (this.peek == AviatorConstants.SINGLE_QUOTATION_MARKS || this.peek == AviatorConstants.SLASH_RIGHT) {
            char left = this.peek;
            int startIndex = this.iterator.getIndex();
            StringBuilder sb = new StringBuilder();
            // char prev = this.peek;
