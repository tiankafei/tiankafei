...
import com.googlecode.aviator.constants.AviatorConstants;
...
public class ExpressionParser implements Parser {
    ...
    public void ternary() {
        join();
        if (this.lookhead == null || expectChar(AviatorConstants.COLON) || expectChar(AviatorConstants.COMMA)) {
            return;
        }
        if (expectChar(AviatorConstants.QUESTION_MARK)) {
            move(true);
            this.codeGenerator.onTernaryBoolean(this.lookhead);
            ternary();
            if (expectChar(AviatorConstants.COLON)) {
                move(true);
    ...
    public void unary() {
        if (expectChar(AviatorConstants.EXCLAMATION_MARK)) {
            move(true);
            // check if it is a seq function call,"!" as variable
            if (expectChar(AviatorConstants.COMMA) || expectChar(AviatorConstants.RIGHT_BRACKET)) {
                back();
                factor();
            } else {
                unary();
        ...
        } else if (expectChar(AviatorConstants.HORIZONTAL_LINE)) {
            move(true);
            // check if it is a seq function call,"!" as variable
            if (expectChar(AviatorConstants.COMMA) || expectChar(AviatorConstants.RIGHT_BRACKET)) {
                back();
        ...
        } else if (expectChar(AviatorConstants.WAVY_LINE)) {
            move(true);
            // check if it is a seq function call,"~" as variable
            if (expectChar(AviatorConstants.COMMA) || expectChar(AviatorConstants.RIGHT_BRACKET)) {
                back();
        ...
        while (expectChar(AviatorConstants.LEFT_MIDDLE_BRACKET) || expectChar(AviatorConstants.LEFT_BRACKET)) {
            if (expectChar(AviatorConstants.LEFT_MIDDLE_BRACKET)) {
                // (...)[index]
                arrayAccess();
            } else if (expectChar(AviatorConstants.LEFT_BRACKET)) {
                ...
                if (!expectChar(')')) {
                    ...
                    while (expectChar(AviatorConstants.COMMA)) {
                        move(true);
    ...
    public boolean isOpVariable(final Token<?> token) {
        ...
        if (expectChar(AviatorConstants.COMMA) || expectChar(AviatorConstants.RIGHT_BRACKET)) {
            ...
            if (AviatorConstants.getHorizontalLine().equals(lexeme)) {
                lexeme = "-sub";
            }
    ...
    public void factor() {
        ...
        if (expectChar(AviatorConstants.LEFT_BRACKET)) {
            this.parenDepth++;
            this.depthState.add(DepthState.Parent);
            move(true);
            ternary();
            if (expectChar(AviatorConstants.RIGHT_BRACKET)) {
                move(true);
                this.parenDepth--;
                this.depthState.removeLast();
            }
            ...
            if (prev.getType() == TokenType.Variable && expectChar(AviatorConstants.LEFT_BRACKET)) {
        ...
        } else if (expectChar(AviatorConstants.SLASH_LEFT)) {
            pattern();
        } else {
            reportSyntaxError("invalid value");
        }
    ...
    private void lambda() {
        ...
        if (expectChar(AviatorConstants.RIGHT_BRACKET)) {
            ...
            this.depthState.removeLast();
            move(true);
            if (expectChar(AviatorConstants.HORIZONTAL_LINE)) {
                move(true);
                if (expectChar(AviatorConstants.MORE_THAN_THE)) {
                    this.codeGenerator.onLambdaBodyStart(this.lookhead);
                    move(true);
                    statement();
                    if (this.lookhead != null && this.lookhead.getType() == TokenType.Variable
                            && "end".equals(this.lookhead.getLexeme())) {
    ...
    private void arrayAccess() {
        ...
        while (expectChar(AviatorConstants.LEFT_MIDDLE_BRACKET)) {
            if (!hasArray) {
    ...
    private void array() {
        ...
        ternary();
        if (expectChar(AviatorConstants.RIGHT_MIDDLE_BRACKET)) {
            this.bracketDepth--;
            this.depthState.removeLast();
            move(true);
    ...
    public static final boolean isJavaIdentifier(final String id) {
        ...
        if (AviatorConstants.NULL.equals(id)) {
            return false;
        }
        return true;
    }
    ...
    private void pattern() {
        ...
        while (this.lookhead != null) {
            while (!expectChar(AviatorConstants.SLASH_LEFT)) {
                sb.append(this.lookhead.getLexeme());
                move(false);
            }
            if (this.prevToken.getType() == TokenType.Char
                    && "\\".equals(((CharToken) this.prevToken).getLexeme())) {
                sb.append("/");
                move(false);
                continue;
            }
    ...
    private void statement() {
        ternary();
        ensureDepthState();
        while (expectChar(AviatorConstants.SEMICOLON)) {
