...
public class TypeUtils {
    ...
    public static BigDecimal ln(Map<String, Object> env, BigDecimal x) {
        ...
        for (long i = NEWTON_METHOD_REPEATS; i >= 0; i--) {
            BigDecimal bigDecimal = new BigDecimal(i / 2 + 1).pow(2);
            bigDecimal = bigDecimal.multiply(x, mathContext);
            ret = bigDecimal.divide(ret, mathContext);

            bigDecimal = new BigDecimal(i + 1);
            ret = ret.add(bigDecimal, mathContext);
