 ...
 * @author tiankafei
 */
public abstract class BaseAbstFelNode extends CommonTree implements FelNode, Interpreter {
    ...
    public List<FelNode> getChildren() {
        List<Object> objectList = this.children;
        if (objectList != null && !objectList.isEmpty()) {
            int length = objectList.size();
            List<FelNode> felNodeList = new ArrayList<FelNode>(length);
            for (int index = 0; index < length; index++) {
                FelNode felNode = (FelNode) objectList.get(index);
                felNodeList.add(felNode);
            }
            return felNodeList;
        } else {
            return new ArrayList<FelNode>();
        }
    }

    public BaseAbstFelNode(Token token) {
        super(token);
    }

    public BaseAbstFelNode() {
...
//		resetSourceBuilder();
    ...
    @SuppressWarnings("unchecked")
    public BaseAbstFelNode(CommonTree node) {
        super(node);
        if (node.getChildren() != null) {
            this.children = new ArrayList<Object>(node.getChildren());
    ...
    @Override
    public Object eval(FelContext context) throws Exception {
        if (this instanceof VarAstNode) {
            VarAstNode varAstNode = (VarAstNode) this;
            if (context.existsKey(varAstNode.getText())) {
                return interpreter.interpret(context, this);
            } else {
                throw new Exception("请确认规则表达式中的参数是否存在值！");
            }
        }
        Object object = interpreter.interpret(context, this);
        context.clearValue();
        return object;
        //		if (cached) {
        //			return cacheValue;
