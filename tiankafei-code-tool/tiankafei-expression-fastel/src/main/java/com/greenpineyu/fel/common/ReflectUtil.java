...
public class ReflectUtil {
     ...
     */
    static final Map<Class<?>, Class<?>> WRAPPER_CLS;
    /**
     * key为基本类型及其包装类型，value为基本类型
     */
    static final Map<Class<?>, Class<?>> PRIMITIVE_CLS;

    static final Map<Class<?>, Class<?>> NUMBER_CLASS_MAP;

    static {
        NUMBER_CLASS_MAP = wrapperNumberCls();
        PRIMITIVE_CLS = new HashMap<Class<?>, Class<?>>(primitiveNumberCls());
        PRIMITIVE_CLS.put(boolean.class, boolean.class);
        PRIMITIVE_CLS.put(Boolean.class, boolean.class);

        WRAPPER_CLS = new HashMap<Class<?>, Class<?>>(wrapperNumberCls());
        WRAPPER_CLS.put(boolean.class, Boolean.class);
        WRAPPER_CLS.put(Boolean.class, Boolean.class);
    }

    private static Map<Class<?>, Class<?>> primitiveNumberCls() {
        Map<Class<?>, Class<?>> map = new HashMap<Class<?>, Class<?>>(14);
    ...
    public static boolean isPrimitiveOrWrapNumber(Class<?> c) {
        return NUMBER_CLASS_MAP.containsKey(c);
    }
...
//		return NUMBER_CLASS_MAP.containsKey(c);
    ...
    public static Class<?> toWrapperClass(Class<?> c) {
        return WRAPPER_CLS.get(c);
    }

    public static Class<?> toPrimitiveClass(Class<?> c) {
        return PRIMITIVE_CLS.get(c);
    }
