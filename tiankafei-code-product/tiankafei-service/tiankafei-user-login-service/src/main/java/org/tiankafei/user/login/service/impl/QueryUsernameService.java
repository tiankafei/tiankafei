...
public class QueryUsernameService implements QueryUserService {
    ...
    public Boolean checkUserExists(String keywords) throws LoginException {
        SysUserLoginQueryVo sysUserLoginQueryVo = loginMapper.queryUserLoginFromUsername(new LoginParamVo().setKeywords(keywords));
        return sysUserLoginQueryVo != null;
    }

    @Override
    public SysUserLoginQueryVo login(String keywords, String password) throws LoginException {
        SysUserLoginQueryVo sysUserLoginQueryVo = loginMapper.queryUserLoginFromUsername(new LoginParamVo().setKeywords(keywords).setPassword(password));
