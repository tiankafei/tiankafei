...
public class QueryMoreService implements QueryUserService {
    ...
    public Boolean checkUserExists(String keywords) throws LoginException {
        SysUserLoginQueryVo sysUserLoginQueryVo = loginMapper.queryUserLogin(new LoginParamVo().setKeywords(keywords));
        return sysUserLoginQueryVo == null;
    }

    @Override
    public SysUserLoginQueryVo login(String keywords, String password) throws LoginException {
        SysUserLoginQueryVo sysUserLoginQueryVo = loginMapper.queryUserLogin(new LoginParamVo().setKeywords(keywords).setPassword(password));
